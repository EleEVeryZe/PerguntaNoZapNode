...
export default class Question {
    ...
    getNextQuestionByNumberId = async () : Promise<IQuestion> => {
        const response = await axiosInstance.get("http://localhost:3003/bigball/next_question", { params: { numberId: this.numberId } });
    ...
    answer = (questionId: string) => {
        return {
            with: async (answer: string) => {
                console.log("START: call answer endpoint with ", { numberId: this.numberId, answer, questionId }); //TODO: implement a better logging library
                const result = await axiosInstance.post("http://localhost:3003/bigball/answer", { numberId: this.numberId, answer, questionId }); //TODO: remove hardcoded url
            ...
            try {
                ...
                if (err.response){
                    ...
                    if (err.response.status === 400)
                        this.whatsAppClient.reply(this.getErrorMsg(err.response.data.errMsg) + this.iteration.question.text);
                }
                return;
            }
        }
    ...
    private getErrorMsg = async (err: any) => {
