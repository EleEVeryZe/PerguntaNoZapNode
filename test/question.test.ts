...
describe("Questions", () => {
    ...
    let whatsAppClientMock = { reply: (msg: string) => {} }
    ...
    test("Should get next question from API", async () => {
        ...
        expect(axios.get).toHaveBeenCalledWith("http://localhost:3003/bigball/next_question", {"params": {"numberId": NUMBER_ID}});
    })

    test("Should call answer question API", () => {
        questionService.answer("12").with("this is crazy shit");
        expect(axios.post).toHaveBeenCalledWith("http://localhost:3003/bigball/answer", {"answer": "this is crazy shit", "numberId": "+5541832990", "questionId": "12"});
    })
    ...
    test("Should answer next questions", async () => {
        ...
        (axios.get as jest.Mock).mockResolvedValue({data: { id: "2", text: "Qual horário do jogo?" }})
    ...
    test("Should answer next question wrongly", async () => {
        ...
        mockedAxios.post.mockRejectedValue({ response: { status: 422, data: { errMsg: "Uma msg de erro nada formal" } } })
        ...
        //then
        expect(whatsAppClientSpy).toHaveBeenCalledWith("Não consegui entender. " + "Qual time vai jogar?");
    })
